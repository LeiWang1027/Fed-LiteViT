# --------------------------------------------------------
# FedLiteViT Model Architecture
# Copyright (c) 2022 Microsoft
# Build the FedLiteViT Model
...
class FedLiteViTBlock(torch.nn.Module):    
    """ A basic FedLiteViT building block.
    ...
    def forward(self, x):
        x = self.dw0(x)
        x = self.ffn0(x)
        #print("x.shape-------before-mixer------------------------------------------",x.shape)
        x = self.mixer(x)
        x = self.dw1(x)
        x = self.ffn1(x)
        return x
...
class FedLiteViT(torch.nn.Module):
    def __init__(self, img_size=224,
        ...
        for i, (stg, ed, kd, dpth, nh, ar, wd, do) in enumerate(
            ...
            for d in range(dpth):
                eval('self.blocks' + str(i+1)).append(FedLiteViTBlock(stg, ed, kd, nh, ar, resolution, wd, kernels))
            if do[0] == 'subsample':
                # Build FedLiteViT downsample block
                #('Subsample' stride)
    ...
    def forward(self, x):
        ...
        #print("x.shape-------patch_ew---------------------",x.shape)
        x = self.blocks1(x)
        x = self.blocks2(x)
        x = self.blocks3(x)
